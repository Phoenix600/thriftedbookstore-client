...
class SellerServices {
  void sellProduct({
    ...
    try {
      final cloudinary = CloudinaryPublic('dqixbzrvd', 'potxt1xk');
  ...
  Future<List<Order>> fetchAllOrders(BuildContext context) async {
    ...
    try {
      http.Response res =
          await http.get(Uri.parse('$uri/seller/get-orders'), headers: {
        'Content-Type': 'application/json; charset=UTF-8',
        'x-auth-token': userProvider.user.token,
      });
    ...
    try {
      ...
      httpErrorHandle(
        ...
        onSuccess: () {
          ...
          sales = [
            Sales('Academics', response['academicsEarnings']),
            Sales('Comic', response['comicEarnings']),
            Sales('Fiction', response['fictionEarnings']),
            Sales('Novel', response['novelEarnings']),
            Sales('Collectibles', response['collectiblesEarnings']),
          ];
        },
      );
    } catch (e) {
